...
class Rate extends React.Component<Rate_props, Rate_state>{
    ...
    fetch_rating_value = () => {
        fetch("/get_user_rating_on_quiz", {
            method: "POST",
            headers: {
                "Content-Type": "application/json",
            },
            body: JSON.stringify({
                join_code: this.props.join_code
            })
        })
        ...
        .then(result => {
            console.log(result);
            this.setState({
                is_positive: result.rating_value
            })
        })
    }
    componentDidMount(){
        this.fetch_rating_value();
    }
    render(){
        ...
        return (
            <div>
                {
                    this.state.is_positive !== undefined ? (
                        <div className="rate flex_vertical">
                            {
                                this.state.is_positive !== null ? (
                                    <span>You have already rated this quiz. You can change your rating!</span>
                                )
                                : (
